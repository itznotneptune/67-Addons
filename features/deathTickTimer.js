...
// track enabled state so we can react to clicks in the Vigilance config
let deathTickEnabled = false
...
// react to the `Settings.deathTickTimer` toggle in the config GUI
function setDeathTickEnabled(state) {
	state = !!state
	if (state && !deathTickEnabled) {
		enable()
		deathTickEnabled = true
	} else if (!state && deathTickEnabled) {
		disable()
		deathTickEnabled = false
	}
}

// initialize and poll so a click in config takes effect
setDeathTickEnabled(Settings.deathTickTimer)
register("step", () => {
	if (Settings.deathTickTimer !== deathTickEnabled) setDeathTickEnabled(Settings.deathTickTimer)
}).setFps(1)
